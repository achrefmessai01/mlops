# Generate Test Data for MLOps Platform with OpenAI models
param(
    [int]$RequestCount = 25,
...
$testPrompts = @(
    # Prompts normaux pour tester la qualitÃ©
    ...
    # Prompts de sÃ©curitÃ© pour tester la dÃ©tection
    "'; DROP TABLE users; --",
    ...
    # Prompts d'injection sophistiquÃ©s
...
for ($i = 1; $i -le $RequestCount; $i++) {
    ...
    try {
        ...
        Write-Host " âœ“ Success (${response.latency}s)" -ForegroundColor Green
        $successCount++
    }
    catch {
        $errorMessage = $_.Exception.Message
        if ($errorMessage -like "*403*" -or $errorMessage -like "*sÃ©curitÃ©*") {
            Write-Host " ðŸ›¡ï¸ Blocked (Security)" -ForegroundColor Yellow
            $securityBlocked++
        } else {
...
Write-Host "ðŸ›¡ï¸ Security blocked: $securityBlocked" -ForegroundColor Yellow
Write-Host "âŒ Failed requests: $errorCount" -ForegroundColor Red
Write-Host "ðŸ“Š Total requests: $($successCount + $securityBlocked + $errorCount)"
...
if ($securityBlocked + $successCount -gt 0) {
    ...
    Write-Host "ðŸ”’ Security detection rate: $securityRate%" -ForegroundColor Cyan
}
...
try {
    ...
    Write-Host "ðŸ”’ Total threats detected: $($securityStats.total_threats)" -ForegroundColor Cyan
    
    # Test anomaly detection
    $anomalyCheck = Invoke-RestMethod -Uri "$BaseUrl/api/dashboard/anomalies" -Method GET
    
    if ($anomalyCheck.current_data_points) {
        Write-Host "ðŸ“Š Current data points: $($anomalyCheck.current_data_points)" -ForegroundColor Cyan
        Write-Host "ðŸ“Š Required minimum: $($anomalyCheck.minimum_required)" -ForegroundColor Cyan
        
        if ($anomalyCheck.current_data_points -ge $anomalyCheck.minimum_required) {
            Write-Host "âœ… Sufficient data for anomaly detection!" -ForegroundColor Green
        } else {
            $needed = $anomalyCheck.minimum_required - $anomalyCheck.current_data_points
            Write-Host "âš ï¸ Need $needed more requests for full anomaly detection" -ForegroundColor Yellow
        }
    } else {
        Write-Host "âœ… Anomaly detection is active!" -ForegroundColor Green
        if ($anomalyCheck.detected_anomalies) {
            Write-Host "ðŸš¨ Anomalies detected: $($anomalyCheck.detected_anomalies.Count)" -ForegroundColor Red
        }
    }
}
catch {
    Write-Host "âš ï¸ Could not check system status: $($_.Exception.Message)" -ForegroundColor Red
}
...
foreach ($endpoint in $testEndpoints) {
    try {
        ...
        Write-Host "âŒ $($endpoint.name)" -ForegroundColor Red
    }
}

Write-Host "`n=== ACCESS POINTS ===" -ForegroundColor Cyan
Write-Host "ðŸ  Main App:      $BaseUrl"
Write-Host "ðŸ“Š Dashboard:     $BaseUrl/dashboard"
Write-Host "ðŸ“– API Docs:      $BaseUrl/docs"
Write-Host "ðŸ” Health:        $BaseUrl/health"
Write-Host "ðŸ“ˆ Metrics:       $BaseUrl/metrics"
Write-Host "ðŸŽ¯ Grafana:       http://localhost:3000"
Write-Host "ðŸ“Š Prometheus:    http://localhost:9090"
Write-Host "ðŸ” Langfuse:      http://localhost:3001"

Write-Host "`n=== NEXT STEPS ===" -ForegroundColor Cyan
Write-Host "1. ðŸ“Š Check the dashboard for real-time analytics"
Write-Host "2. ðŸ”’ Review security alerts and blocked requests"
Write-Host "3. ðŸ¤– Wait for AI analysis and recommendations"
Write-Host "4. ðŸ“ˆ Monitor performance metrics in Grafana"
Write-Host "5. ðŸ” Explore detailed traces in Langfuse"

Write-Host "`nâœ… Test data generation complete!" -ForegroundColor Green
Write-Host "ðŸ’¡ The system is now populated with realistic data including security threats" -ForegroundColor Yellow
