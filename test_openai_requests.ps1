...
$testPrompts = @(
    # Prompts normaux pour tester la qualité
    ...
    # Prompts de sécurité pour tester la détection
    "'; DROP TABLE users; --",
    ...
    # Prompts d'injection sophistiqués
...
for ($i = 1; $i -le $RequestCount; $i++) {
    ...
    try {
        ...
        Write-Host " ✓ Success (${response.latency}s)" -ForegroundColor Green
        $successCount++
    }
    catch {
        $errorMessage = $_.Exception.Message
        if ($errorMessage -like "*403*" -or $errorMessage -like "*sécurité*") {
            Write-Host " 🛡️ Blocked (Security)" -ForegroundColor Yellow
            $securityBlocked++
        } else {
...
Write-Host "🛡️ Security blocked: $securityBlocked" -ForegroundColor Yellow
...
try {
    ...
    if ($anomalyCheck.current_data_points) {
        ...
        if ($anomalyCheck.current_data_points -ge $anomalyCheck.minimum_required) {
            ...
            Write-Host "⚠️ Need $needed more requests for full anomaly detection" -ForegroundColor Yellow
        }
    } else {
    ...
    Write-Host "⚠️ Could not check system status: $($_.Exception.Message)" -ForegroundColor Red
}
